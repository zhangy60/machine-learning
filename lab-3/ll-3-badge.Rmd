...
-   Before the tuning, the accuracy value is about 80%, and it was boostered to 88% after tuning. Tuning seems to work well and defnitely worth the investment.
...
-   Kappa is a measure of classification accuracy. Kappa = (observed accuracy - expected accuracy)/(1 - expected accuracy)
...
2.  What research questions were the authors of this study trying to address and why did they consider these questions important?
    ...
    -   The authors were interested in the prediciton of the performance of Moroccan students using an intelligent system through three methods: linear regression, decision tree, and random forest.

3.  What were the results of these analyses?

    -   The random forest algorithm was found to give the best predictions than the two other algorithms tested.
...
kfcv <- vfold_cv(data_train, k=5) # again, we will use resampling
```
...
```{r panel-chunk-5b, echo = TRUE, eval = FALSE}
...
```{r panel-chunk-6, echo = TRUE, eval = FALSE}
# fit stats
final_fit %>%
    collect_metrics()
